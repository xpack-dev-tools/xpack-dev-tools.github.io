...
version (currently tested up to 15.7).
...
**Any x86_64/aarch64 GNU/Linux distribution
capable of running Docker should suffice**; a physical machine is not
necessary, as virtual machines are
perfectly adequate. For optimal results, allocate 4 cores and 8-12 GB of RAM.
GitHub self-hosted runners may require additional memory.
...
Building Arm binaries necessitates an Arm machine. 
A Raspberry Pi 5 with an NVME SSD is an excellent solution.

:::

The prerequisites are:

- `uname` (usually in the base system)
